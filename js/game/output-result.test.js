...
describe(`Get output result`, () => {
  it(`should get the game of result`, () => {
    ...
    assert.equal(`У вас закончились все попытки. <br> Ничего, повезёт в следующий раз!`, outputGameResult(STATISTICS, GAME_RESULT[1]));
    assert.equal(`Время вышло! <br> Вы не успели отгадать все мелодии`, outputGameResult(STATISTICS, GAME_RESULT[2]));
